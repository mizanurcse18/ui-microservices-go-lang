...
export function SidebarMenu() {
  ...
  // Extract values from context, but handle the case where context is still loading
  const menuItems = menuContext?.menuItems;
  const isLoading = menuContext?.isLoading;
  const error = menuContext?.error;
  
  // During initial loading, if we don't have dynamic menu items yet, show loading state
  // rather than falling back to static menu immediately
  ...
  // Debug logging to see what menu items are being used
  useEffect(() => {
    console.log('SidebarMenu received menu items:', finalMenuItems?.length, 'items');
    if (finalMenuItems && finalMenuItems.length > 0) {
      console.log('First menu item in sidebar:', finalMenuItems[0]);
    }
  }, [finalMenuItems]);
  ...
  return (
    <div className="kt-scrollable-y-hover flex grow shrink-0 py-5 px-5 lg:max-h-[calc(100vh-5.5rem)]">
      ...
      {error && (
        <div className="flex items-center justify-center w-full h-20">
          <div className="text-sm text-red-500">Failed to load menu: {error}</div>
        </div>
      )}
